...
def expand_node(node, priors):
    ...
    for action in valid_moves(board):
        ...
        child = make_mcts_node(prior = float(priors[action]),parent=node)

        child["board"] = child_board
        child["to_play"] = other_player(to_play)
...
def visit_count_policy(root, temperature=1.0):
    ...
    policy = scaled_visits / scaled_visits.sum()
    return policy
...
def mcts_choose_action(state, to_play, net, num_simulations, c_puct, temperature=1.0):
    ...
    probs = torch.tensor(policy, dtype=torch.float32)
